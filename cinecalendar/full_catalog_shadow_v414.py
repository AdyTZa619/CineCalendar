...
FULL_CATALOG_RETRIEVAL_VERSION = "full-catalog-retrieval-v4.14.0"
SHADOW_EVALUATION_VERSION = "retrieval-shadow-v4.14.0"
MAX_SIGNALS = 48
PER_SIGNAL_LIMIT = 90
...
class FullCatalogCandidateGeneratorV414:
    ...
    def _query_signal(self, con, feature: str) -> list[int]:
        ...
        if prefix in {"director", "genre", "country"}:
            ...
            # 4.14 probes validated interaction features directly. In 4.13 the profile selected
            # combo signals but retrieval ignored them, so strong director+genre or genre+decade
            # preferences could never seed the candidate pool.
            conditions: list[str] = []
            values: list[object] = []
            for component in value.split("|"):
                ...
                if not kind or not raw:
                    return []
                if kind in {"director", "genre", "country"}:
                    column = {
                        "director": "directors_json",
                        "genre": "genres_json",
                        "country": "countries_json",
                    }[kind]
                    normalized = normalize_text(raw)
                    conditions.append(
                        f"EXISTS (SELECT 1 FROM json_each(COALESCE(m.{column},'[]')) j "
                        "WHERE lower(trim(CAST(j.value AS TEXT)))=?)"
                    )
                    values.append(normalized)
                elif kind == "decade" and raw.endswith("s") and raw[:-1].isdigit():
                    start = int(raw[:-1])
                    conditions.append("m.year>=? AND m.year<?")
                    values.extend((start, start + 10))
                elif kind == "runtime":
                    ranges = {
                        "<90": "m.runtime_min<90",
                        "90-120": "m.runtime_min BETWEEN 90 AND 120",
                    ...
                    condition = ranges.get(raw.lower())
                    if condition is None:
                        return []
            ...
            if not conditions:
                return []
            query = base + " AND " + " AND ".join(f"({item})" for item in conditions)
            params = tuple(values)
        else:
            return []
...
class FullCatalogShadowEvaluatorV414:
    ...
    def __init__(self, db, collaborative, baseline_engine: str):
        self.db = db
        self.generator = FullCatalogCandidateGeneratorV414(db, collaborative)
    ...
    def status(self) -> dict:
        from .full_catalog_evaluation_v414 import live_shadow_metrics

        with self.db.connect() as con:
            total = int(
                con.execute(
                    "SELECT COUNT(*) FROM retrieval_shadow_runs WHERE challenger_version=?",
                    (FULL_CATALOG_RETRIEVAL_VERSION,),
                ).fetchone()[0]
                or 0
            )
        ...
        measured = live_shadow_metrics(
            self.db,
            challenger_version=FULL_CATALOG_RETRIEVAL_VERSION,
        )
        return {
            ...
            "baseline": dict(measured.get("baseline") or {}),
            "challenger": dict(measured.get("challenger") or {}),
            "paired": dict(measured.get("paired") or {}),
            "eligible_for_offline_review": bool(measured.get("eligible_for_offline_review")),
            "minimum_challenger_ratings": int(measured.get("minimum_challenger_ratings", 20) or 20),
            "minimum_paired_runs": int(measured.get("minimum_paired_runs", 8) or 8),
            "promotion_allowed_from_live_shadow": False,
            "measurement_limitation": str(measured.get("limitation") or ""),
            "evaluation_version": str(measured.get("version") or ""),
