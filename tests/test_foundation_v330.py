...
def test_partial_v5_migration_is_resumed_and_upgraded_to_current_schema(tmp_path):
    ...
    db = Database(path)
    with db.connect() as con:
        assert con.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0] == 12
        assert con.execute(
        ...
        assert con.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='imdb_rating_followups'"
        ).fetchone() is not None
